...
export default function ClassificationsPage() {
  ...
  const fetchSchemas = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/v1/admin/schemas?type=classification`, {
        headers: authHeaders(),
      });
      if (!res.ok) throw new Error("Failed to load classification schemas");
      const data: SchemasResponse = await res.json();
      setSchemas(data.data ?? []);
    } catch (err) {
      showToast(err instanceof Error ? err.message : "Failed to load classifications", "error");
    } finally {
      setLoading(false);
    }
  }, [showToast]);
    ...
    <tr>
      <td colSpan={5}>
        <div className="flex flex-col items-center justify-center py-16 text-surface-500">
          ...
          <p className="text-sm font-medium">No classification schemas found</p>
          <p className="text-xs mt-1">Configure classification schemas to appear here</p>
        </div>
      </td>
    </tr>
  );
  ...
  return (
    ...
    <div className="space-y-6">
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Classifications</h1>
        <p className="text-sm text-surface-400 mt-1">Dialog classification schemas and results</p>
      </div>
