...
function ViewDialog({ schema, onClose }: { schema: Schema; onClose: () => void }) {
  const t = useTranslations("settings.classifications");
...
export default function ClassificationsPage() {
  ...
  const fetchSchemas = useCallback(async () => {
    ...
    try {
      const data = await get<{ data: Schema[] }>("/v1/admin/schemas?type=classification");
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-surface-800">
              {loading ? (
                ...
                <tr><td colSpan={5}><EmptyState icon={Tags} title={t("emptyTitle")} description={t("emptyDescription")} /></td></tr>
              ) : (
